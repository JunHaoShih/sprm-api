...
using SprmAuthentication.Core.AppUsers.Dto;
...
namespace SprmAuthentication.Core.AppUsers
{
    ...
    public class AppUserController : ControllerBase
    {
        ...
        {
            OffsetPagination<AppUser> usersPagination = _appUserService.GetByPattern(pattern, input);
            List<AppUser> pagingList = await usersPagination.GetPagedListAsync();
            IEnumerable<AppUserDto> dtos = pagingList.Select(appUser => AppUserDto.Parse(appUser));
            _paginationData.PaginationHeader = usersPagination.GetResponseHeader();
            return Ok(GenericResponse<IEnumerable<AppUserDto>>.Success(dtos));
        }
    }
}
