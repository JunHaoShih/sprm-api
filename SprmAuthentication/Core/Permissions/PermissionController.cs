...
namespace SprmAuthentication.Core.Permissions
{
    ...
    public class PermissionController : ControllerBase
    {
        ...
        {
            IEnumerable<Permission> permissions = await _permissionService.GetByUserIdAsync(userId);
            IEnumerable<PermissionDto> dtos = permissions.Select(permission => permission.ToDto());
            return Ok(GenericResponse<IEnumerable<PermissionDto>>.Success(dtos));
        }
        ...
        {
            IEnumerable<Permission> permissions = await _permissionService.GetByUserNameAsync(_headerData.JWTPayload.Subject);
            IEnumerable<PermissionDto> dtos = permissions.Select(permission => permission.ToDto());
            return Ok(GenericResponse<IEnumerable<PermissionDto>>.Success(dtos));
        }
    }
